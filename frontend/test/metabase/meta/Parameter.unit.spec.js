...
describe("metabase/meta/Parameter", () => {
  ...
  describe("parameterToMBQLFilter", () => {
    ...
    it("should return null if the parameter has no target or the target is not a field dimension", () => {
      const parameterWithNoTarget = {};
      expect(parameterToMBQLFilter(parameterWithNoTarget, metadata)).toBe(null);

      const parameterWithNonDimensionTarget = { target: ["foo"] };
      expect(
        parameterToMBQLFilter(parameterWithNonDimensionTarget, metadata),
      ).toBe(null);

      const parameterWithNonArrayDimension = { target: ["dimension", 123] };
      expect(
        parameterToMBQLFilter(parameterWithNonArrayDimension, metadata),
      ).toBe(null);

      const parameterWithTemplateTagTarget = {
        target: ["dimension", ["template-tag", "foo"]],
      };
      expect(
        parameterToMBQLFilter(parameterWithTemplateTagTarget, metadata),
      ).toBe(null);
    });

    it("should convert a date parameter into an mbql filter", () => {
      const dateParameter = {
        type: "date/single",
        value: "2017-05-01",
      ...
      expect(parameterToMBQLFilter(dateParameter, metadata));
    });

    it("should convert a number parameter into an mbql filter", () => {
      ...
      expect(parameterToMBQLFilter(numberParameter, metadata)).toEqual([
        "=",
        ["field", 123, null],
        1,
        2,
    ...
    it("should convert a string parameter into an mbql filter", () => {
      ...
      expect(parameterToMBQLFilter(stringParameter, metadata)).toEqual([
        "contains",
        ["field", 123, null],
        "foo",
      ]);
    ...
    it("should convert a category parameter into an mbql filter", () => {
      ...
      expect(parameterToMBQLFilter(categoryParameter, metadata)).toEqual([
        "=",
        ["field", 123, null],
        true,
      ]);
  ...
  describe("getParameterIconName", () => {
    it("should return an icon string depending on the parameter type", () => {
      expect(getParameterIconName({ type: "number/!=" })).toEqual("number");
    });

    it("should default", () => {
      expect(getParameterIconName({ type: "doesntexist" })).toEqual("label");
    });
  });
